...
"""
    Program which splits MusicXML files into SPEAC
    identifiers by beat. MusicXML files must be in the
    key of C or A minor for this to work correctly.
"""
...
if(options.filename):
    ...
    print("Usage: python3 SPEACSplitter.py -f [PATH TO FILE]")
    sys.exit()
...
def splitBeats(measure,divisions,beats,staves):
    ...
    for note in measure.findall("./note/"):
        ...
        if note.tag == "pitch":
            ...
            duration = int(note.text)/divisions

        elif note.tag == "staff":
            staff = int(note.text)
            everynote.append((notestr, duration, staff))
    ...
    for note in everynote:
        if currentbeat > beats-1 or note[2] != clef:
            currentbeat = 0
            clef = note[2]
        beatlist[currentbeat].append(note[0][1])
...
if __name__ == '__main__':
    ...
    for measure in measurelist:
        beatlist = splitBeats(measure,int(divisions),int(beats),int(staves))
